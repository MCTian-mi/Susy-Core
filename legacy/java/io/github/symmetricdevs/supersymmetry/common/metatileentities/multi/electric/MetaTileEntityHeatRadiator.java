...
import io.github.symmetricdevs.supersymmetry.common.blocks.BlockSerpentine;
...
public class MetaTileEntityHeatRadiator extends WorkableElectricMultiblockMachine {
    ...
    protected boolean updateStructureDimensions() {
        ...
        Direction up = UP.getRelativeFacing(this.getFrontFacing(), this.getUpwardsFacing(), this.isFlipped()); // From
                                                                                                                // the
                                                                                                                // flare
                                                                                                                // stack,
                                                                                                                // I
                                                                                                                ...
                                                                                                                // free
                                                                                                                // rotation.
        Direction left = front.rotateAround(up.getAxis());
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        builders.forEach(builder -> shapeInfo.add(builder
                .where('S', SuSyMetaTileEntities.HEAT_RADIATOR, Direction.SOUTH)
                .where('I', MetaTileEntities.FLUID_IMPORT_HATCH[GTValues.LV], Direction.SOUTH)
                .where('O', MetaTileEntities.FLUID_EXPORT_HATCH[GTValues.LV], Direction.SOUTH)
                .where('M',
                        () -> ConfigHolder.machines.enableMaintenance ? MetaTileEntities.MAINTENANCE_HATCH :
                                getCasingState(),
                        Direction.SOUTH)  // From Cleanroom
    ...
    protected void addDisplayText(List<Component> textList) {
        ...
        if (isStructureFormed()) {
            Component componentParallelAmount = TextComponentUtil.stringWithColor(ChatFormatting.DARK_PURPLE,
                    String.valueOf(this.area));
            Component componentParallelAmountBase = TextComponentUtil.translationWithColor(ChatFormatting.GRAY,
                    "susy.machine.heat_radiator.parallel",
                    componentParallelAmount);
            Component componentParallelAmountHover = TextComponentUtil.translationWithColor(ChatFormatting.GRAY,
    ...
    public BlockState getRadiatorElementState() {
    ...
    private class ParallelableNoEnergyMultiblockRecipeLogic extends NoEnergyMultiblockRecipeLogic {
        ...
        public ParallelableNoEnergyMultiblockRecipeLogic(WorkableElectricMultiblockMachine BlockEntity) {
            super(BlockEntity);
        }
