...
import io.github.symmetricdevs.supersymmetry.common.blocks.rocketry.BlockFairingConnector;
...
public class ComponentFairing extends AbstractComponent<ComponentFairing> {
    ...
    @Override
    public Optional<ComponentFairing> readFromNBT(CompoundTag compound) {
        ...
        compound
                ...
                .forEach(x -> fairing.materials.add(MaterialCost.fromNBT((CompoundTag) x)));
    ...
    @Override
    public Optional<CompoundTag> analyzePattern(StructAnalysis analysis, AABB interiorBB) {
        ...
        AABB fairingBB = analysis.getBB(blocksConnected);
        ...
        BlockPos start = next;
        Direction dir = world.getBlockState(next).getValue(FACING);
        int axisPos = analysis.getCoordOfAxis(start);
        for (BlockPos blockFace : connectorBlocks) {
            Direction dir_general = world.getBlockState(blockFace).getValue(FACING);
