...
import com.gregtechceu.gtceu.api.util.BlockUtility;
import io.github.symmetricdevs.supersymmetry.common.blocks.rocketry.*;
import io.github.symmetricdevs.supersymmetry.common.tileentities.SuSyTileEntities;
...
public class SuSyBlocks {
    ...
    public static void registerWalkingSpeedBonus() {
        ...
        for (BlockState state : ASPHALT.getBlockState().getValidStates()) {
            BlockUtility.setWalkingSpeedBonus(state, 1); // Buff from 0.6F
        }
        for (BlockState state : LUNAR_CONCRETE.getBlockState().getValidStates()) {
            BlockUtility.setWalkingSpeedBonus(state, BlockUtility.ASPHALT_WALKING_SPEED_BONUS);
        }
        for (SusyStoneVariantBlock block : SUSY_STONE_BLOCKS.values()) {
            BlockState state = block.getState(SusyStoneVariantBlock.StoneType.INDUSTRIAL_CONCRETE);
            BlockUtility.setWalkingSpeedBonus(state, BlockUtility.ASPHALT_WALKING_SPEED_BONUS);
        }
        for (BlockState state : RANDOM_CONCRETE.getBlockState().getValidStates()) {
            if (isDottedPanel(state)) {
                continue;
            }
            BlockUtility.setWalkingSpeedBonus(state, BlockUtility.ASPHALT_WALKING_SPEED_BONUS);
        }
        for (BlockState state : RANDOM_CONCRETE1.getBlockState().getValidStates()) {
            BlockUtility.setWalkingSpeedBonus(state, BlockUtility.ASPHALT_WALKING_SPEED_BONUS);
        }
    }

    private static boolean isDottedPanel(BlockState state) {
