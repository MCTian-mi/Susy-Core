...
import net.minecraft.world.level.block.BlockFaceShape;
...
import com.gregtechceu.gtceu.common.blocks.properties.PropertyMaterial;
import it.unimi.dsi.fastutil.objects.Object2ObjectOpenHashMap;
import io.github.symmetricdevs.supersymmetry.api.unification.material.info.SuSyMaterialIconType;
...
public class BlockSheetedFrame extends Block {
    ...
    public static enum FrameEnumAxis implements IStringSerializable {
        ...
        public static FrameEnumAxis fromFacingAxis(Direction.Axis axis) {
            switch (axis) {
                case X:
                    return X;
                case Y:
    ...
    @Nonnull
    public SoundType getSoundType(BlockState state, @Nonnull World world, @Nonnull BlockPos pos,
    ...
    public net.minecraft.world.level.material.Material getMaterial(BlockState state) {
        Material material = state.getValue(this.variantProperty);
        return ModHandler.isMaterialWood(material) ? net.minecraft.world.level.material.Material.WOOD :
    ...
    public static ItemStack getItem(BlockState blockState) {
    ...
    public Material getGtMaterial(BlockState state) {
    ...
    /*
     * public boolean replaceWithFramedPipe(World worldIn, BlockPos pos, BlockState state, Player playerIn,
     * ItemStack stackInHand, Direction facing) {
     ...
     * }
     * 
     * public boolean removeFrame(World world, BlockPos pos, Player player, ItemStack stack) {
     ...
     * }
     * 
     * public boolean onBlockActivated(@Nonnull World worldIn, @Nonnull BlockPos pos, @Nonnull BlockState state,
     * Player playerIn, @Nonnull InteractionHand hand, @Nonnull Direction facing, float hitX, float hitY, float hitZ) {
     ...
     * } else {
     * BlockEntity te = worldIn.getTileEntity(blockPos);
     ...
     * worldIn.playSound((Player)null, pos, type.getPlaceSound(), SoundCategory.BLOCKS, (type.getVolume() + 1.0F)
    ...
    public AABB getCollisionBoundingBox(@Nonnull BlockState blockState, @Nonnull IBlockAccess worldIn,
                                                 @Nonnull BlockPos pos) {
        AABB boundingBox = switch (this.getMetaFromState(blockState) >>> 2) {
            // x
            case (0) -> new AABB(0.05, 0.0, 0.00, 0.95, 1.0, 1.00);
            // z
            case (2) -> new AABB(0.00, 0.0, 0.05, 1.0, 1.0, 0.95);
            // NONE (all sided) or y [1] as the climbable axis would be on the top of the block
            default -> new AABB(0.00, 0.0, 0.00, 1.0, 1.0, 1.0);
        };
        return boundingBox;
    }

    @Nonnull
    public BlockFaceShape getBlockFaceShape(@Nonnull IBlockAccess worldIn, @Nonnull BlockState state,
                                            @Nonnull BlockPos pos, @Nonnull Direction face) {
    ...
    @SideOnly(Side.CLIENT)
    public void onModelRegister() {
        Map<BlockState, ModelResourceLocation> map = new Object2ObjectOpenHashMap<>();
        for (BlockState state : this.getBlockState().getValidStates()) {
